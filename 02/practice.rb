...
class Candidate
	attr_reader :name, :interviewers, :email
	def initialize(candidate_hash)
		@name = candidate_hash[:name]
		@email = candidate_hash[:email]
		@interviewers = []

	end

	def AssignInterviewers(array_of_interviewers)
		
		@interviewers << array_of_interviewers.sample
		@interviewers << (array_of_interviewers-interviewers).sample
		#@interviewers
	end
...
class Interviewer
	attr_reader :name
	def initialize(interviewers_hash)
		@name = interviewers_hash[:name]
		@email = interviewers_hash[:email]
	
	end
end

array_of_candidates = CANDIDATES.map{|candidate| Candidate.new(candidate)}

array_of_interviewers = INTERVIEWERS.map {|interviewer| Interviewer.new(interviewer)}
array_of_candidates.each do |candidate| 

	candidate.AssignInterviewers(array_of_interviewers)

end

def email(candidate)
	puts "Sending email to #{candidate.email} \nHola #{candidate.name} \nTus entrevistadores son #{candidate.interviewers[0].name} y #{candidate.interviewers[1].name}"
